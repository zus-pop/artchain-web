  ...
  switch (tierName) {
    ...
    case "gold":
      return {
        ...
        cardClass:
          "border-yellow-200 bg-gradient-to-br from-yellow-50/80 to-white",
    ...
    default:
      return {
        ...
        cardClass:
          "border-orange-200 bg-gradient-to-br from-orange-50/80 to-white",
  ...
  switch (status) {
    case "ACTIVE":
      return {
        ...
        message: "Chiến dịch đang hoạt động và sẵn sàng nhận tài trợ.",
      };
    case "COMPLETED":
      return {
        label: "Đã Kết Thúc",
        className: "bg-gray-100 text-gray-700 border-gray-200",
        message: "Chiến dịch đã kết thúc nên không thể tiếp nhận thêm tài trợ.",
      };
    case "DRAFT":
      return {
        label: "Bản Nháp",
        className: "bg-amber-100 text-amber-700 border-amber-200",
        message: "Chiến dịch đang ở bản nháp nên chưa thể nhận tài trợ.",
      };
    case "CANCELLED":
      return {
        label: "Đã Hủy",
        className: "bg-red-100 text-red-700 border-red-200",
        message: "Chiến dịch đã bị hủy nên không thể tiếp nhận tài trợ.",
      };
    case "CLOSED":
    default:
      return {
        label: "Đã Đóng",
        className: "bg-gray-100 text-gray-700 border-gray-200",
        message: "Chiến dịch đã đóng nên không thể tiếp nhận thêm tài trợ.",
      };
  }
};
...
const sponsorFormSchema = z.object({
  ...
  contactInfo: z.string().trim().min(1, "Vui lòng nhập thông tin liên hệ"),
  sponsorshipAmount: z
    .string()
    .trim()
...
const CampaignDetailPage = () => {
  ...
  const [pendingSponsorData, setPendingSponsorData] =
    useState<CreateSponsorRequest | null>(null);
  ...
  const contactInfoValue = watch("contactInfo") || "";
  const sponsorshipAmountInput = watch("sponsorshipAmount");
  const isCampaignActive = campaign?.status === "ACTIVE";
  const sponsorshipAmountValue = parseSponsorshipAmount(
    sponsorshipAmountInput || "",
  );
  const hasRequiredSponsorData = Boolean(
    sponsorName.trim() &&
    contactInfoValue.trim() &&
    !Number.isNaN(sponsorshipAmountValue) &&
    sponsorshipAmountValue > 0 &&
    logoFile,
  );
  const canSubmitSponsor = Boolean(
    isCampaignActive && hasRequiredSponsorData && !submitting,
  );
  ...
  const handleAmountInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setValue(
      "sponsorshipAmount",
      formatSponsorshipAmountInput(e.target.value),
      {
        shouldDirty: true,
        shouldValidate: true,
      },
  ...
  const handleSubmit = async (values: SponsorFormValues) => {
    if (campaign?.status !== "ACTIVE") {
      setFormError(
        "Chiến dịch hiện không ở trạng thái ACTIVE nên chưa thể nhận tài trợ.",
      );
      return;
    }
  ...
  const activeTier = sponsorshipAmountInput
    ? [...tiers]
        ...
        .find(
          (tier) =>
            parseSponsorshipAmount(sponsorshipAmountInput) >= tier.minPrice,
        )
    : null;
  const campaignStatusConfig = campaign
    ? getCampaignStatusConfig(campaign.status)
    : null;
      ...
      <div className="">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <Link
              href="/campaigns"
              className="flex items-center text-gray-600 hover:text-gray-900 font-bold text-sm"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loadingCampaign ? (
          <div className="text-center py-8">
            <Loader />
            <p className="mt-4 text-sm font-bold animate-pulse text-[#FF6E1A]">
            ...
            <div className="mb-12">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-bold text-gray-900 uppercase tracking-tight border-b-2 border-[#FF6E1A] inline-block pb-1">
                    Hạng Mục Tài Trợ
                  </h3>
                  <div className="space-y-3">
                    {tiers.length > 0 ? (
                      tiers.map((tier) => (
                        <div
                          ...
                          onClick={() => {
                            setValue(
                              "sponsorshipAmount",
                              formatSponsorshipAmountInput(
                                tier.minPrice.toString(),
                              ),
                              {
                                shouldDirty: true,
                                shouldValidate: true,
                        ...
                        >
                          <div className="flex justify-between items-center">
                            ...
                            >
                              {React.createElement(
                                getTierConfig(tier.tierName).Icon,
                                {
                      ...
                      <p className="text-sm text-gray-500 italic">
                        Không có thông tin hạng mục tài trợ.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="mb-4 flex flex-wrap items-center gap-3">
                ...
                {campaignStatusConfig && (
                  <span
                    className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-widest ${campaignStatusConfig.className}`}
                  >
              ...
              {campaignStatusConfig && campaign.status !== "ACTIVE" && (
                <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                  <p className="text-sm font-bold text-amber-800">
                    {campaignStatusConfig.message}
                  </p>
                </div>
              )}
                ...
                <form onSubmit={handleFormSubmit(handleSubmit)}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      ...
                      <button
                        type="submit"
                        disabled={!canSubmitSponsor}
                        className="w-full bg-[#FF6E1A] hover:bg-[#FF833B] disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold py-4 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center text-sm uppercase tracking-widest"
                      >
                        {submitting ? (
                          <Loader2 className="w-5 h-5 animate-spin" />
                        ) : !hasRequiredSponsorData ? (
                          "Điền Đủ Thông Tin Để Tiếp Tục"
                        ) : (
                          "Xác Nhận Tài Trợ"
                        )}
                      </button>
            ...
            <>
              ...
              <span
                className={`font-black ${
                  confirmMeta.tierName
                    ? getTierConfig(confirmMeta.tierName).iconClass
                    : "text-gray-600"
                }`}
              >
