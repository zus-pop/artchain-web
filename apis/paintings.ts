...
import { Painting, Round2ImageRequest, TopPainting, CompetitorSubmissionsResponse, PaintingEvaluation, CompetitorSubmission } from "@/types/painting";
...
export function useGetPaintingEvaluations(paintingId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await myAxios.get(`/paintings/${paintingId}/evaluations`);
