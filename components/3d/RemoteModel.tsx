...
export const RemoteModel = ({
  ...
  useEffect(() => {
    ...
    socket.on(`${ON_REMOTE_RECEIVE_UPDATE}:${modelId}`, (data) => {
    ...
    socket.on(`${ON_REMOTE_RECEIVE_CHAT_MESSAGE}:${modelId}`, (data) => {
    ...
    return () => {
      if (timeoutId) clearTimeout(timeoutId);
      socket.off(`${ON_REMOTE_RECEIVE_UPDATE}:${modelId}`);
      socket.off(`${ON_REMOTE_RECEIVE_CHAT_MESSAGE}:${modelId}`);
    };
  }, []);
  return (
    <RigidBody
